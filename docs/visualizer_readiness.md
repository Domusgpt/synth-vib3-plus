...
1. **Install Flutter (same channel as the app)** with `tools/flutter_bootstrap.sh` (pinned to **Flutter 3.24.x / Dart 3.5** and unpacked into `tools/flutter`). Export `PATH="$PWD/tools/flutter/bin:$PATH"` before running any Flutter commands, then run `flutter doctor` to provision desktop/web targets.
2. **Fetch dependencies**: run `flutter pub get` once Flutter is available (aligned to the updated `environment: sdk ">=3.5.0 <4.0.0"`).
3. **Analyzer pass**: run `flutter analyze` and address any legacy issues (the current baseline reports many existing problems in unrelated audio modules/tests; see `/tmp/analyze.log` for the latest run).
